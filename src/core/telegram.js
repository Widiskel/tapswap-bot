...
export class Telegram {
  ...
  constructor() {
    ...
    this.url = "https://app.tapswap.club/bot=app_bot_1";
  }
  ...
  async resolvePeer() {
    try {
      ...
      while (this.peer == undefined) {
        try {
          this.peer = await this.client.getEntity("tapswap_mirror_bot");
          break;
        } catch (error) {
  ...
  async initWebView() {
    try {
      ...
      console.log(authUrl);
      return [Helper.getTelegramQuery(authUrl, 3), authUrl];
    } catch (error) {
      throw error;
    }
  }
